...
- `[ ]` **S6 `colony-polish-impl`** ✅ *(optional)* — viewport-cull/sprite-budget hardening, district inspect panel (reuse saved-variant naming UI), big-map draw-count verification (the perf lever); also a deferred cosmetic: brushed plant-parent-with-holes canopy gradient. *dep: S3 ✓ S4 ✓ S5 ✓ — OPTIONAL polish/perf.*
