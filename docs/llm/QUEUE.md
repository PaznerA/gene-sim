...
- `[x]` **SB1 sbol-model-validator-impl** — **DONE (2026-06-30, ADR-037)** — new `crates/sbol` (std+serde+serde_json only, NO RDF/network/GPL): the SBOL3-subset model (`SbolDocument`/`Component`/`Feature`/`Sequence`/`Range`/`Interaction`/`Participation`/`Constraint`, ordered `IriId` interner, no HashMap) wrapping `SoTermId`/`DnaSequence` + the in-core deterministic closed-world validator behind the inv #5 trait (rejects the 5 §3.2 conditions; 13 tests). **Hash-neutral + UNWIRED** (grep confirms nothing references it; gate at SB2). **Gate GREEN; `0x47a0` byte-identical; verify 3/3.** Merged `--no-ff`. SBOL3 v3.1.0 + RFC10 + SO/SBO pinned.
