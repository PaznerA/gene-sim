...
- `[ ]` **discovery-steered-loop-impl** — D3-B.4: wire RidgeInt into D2b (oversample→predict→select, explore floor), retrain per gen. *dep: ridgeint ✓ — READY NEXT.* Composes with the Variant Lab D edit axis + the named scenario spaces. **⚠ Behavioural gate: this is where the steering target (drama `D` vs raw `Q`) + model choice actually change search output — surface the design to the human for sign-off before D4 (per ADR-033/034 + spec open-questions).**
