...
- `[ ]` **discovery-ridgeint-impl** — D3-B.3: integer ridge regressor (fixed-point GD, no f64, row-order-independent, `build_id` anchor) trained on `drama_target` of each `EvalRecord` breakdown. *dep: dramaweights ✓ — READY NEXT.*
